...
/* add by Jose (start)*/
	...
	public function field_checkbox($name, $options = array(), $checked=array(), $extra = array())
	{
		$str='<div class="checkbox-group">';
		foreach($options as $key=>$value){
			$str.='<div class="checkbox-item">'.form_checkbox($name,$key,in_array($key,$checked)?TRUE:FALSE) .$value.'</div>';
...
/* add by Jose (start)*/
	...
	{
		return '<div class="form-group">'.form_label($label, $name).$this->field_recaptcha().'</div>';
	}	
/* add by Jose (end) */
